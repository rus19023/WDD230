  ...
  .then(function (jsonObject) { 
    ...
    towns.forEach(town => {
      if (town.name=="Preston"||town.name=="Soda Springs"||town.name=="Fish Haven") {
        ...
        div1.appendChild(image);  //  add the photo to the image div

        // set up cards for each town in order of menu

        if (town.name=="Preston") {
          document.querySelector('#preston').appendChild(div1);  //  set the Preston div into the html page
        } else if (town_name=="Soda Springs") {
        document.querySelector('#sodasprings').appendChild(div1);  //  set the card div into the html page
        } else if (town_name=="Fish Haven") {
        document.querySelector('#fishhaven').appendChild(div1);  //  set the card div into the html page
        }
    }
    });
  });